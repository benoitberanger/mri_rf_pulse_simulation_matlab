classdef simulation_results < mri_rf_pulse_sim.backend.base_class
    ...
    methods (Access = public)
        ...
        function opengui(self, use_onefig)
            ...
            pos_Mxyz          = [0.00 0.70 1.00 0.30];
            pos_SliceProfile  = [0.00 0.35 1.00 0.35];
            pos_ChemicalShift = [0.00 0.00 1.00 0.35];
            ...
            pos_axes_Mxyz           = [0.00 0.00 0.60 1.00];
            pos_axes_Sphere         = [0.60 0.00 0.30 1.00];
            pos_pushbutton_fft_perp = [0.90 0.50 0.10 0.50];
            pos_pushbutton_fft_para = [0.90 0.00 0.10 0.50];

            time = self.app.pulse_definition.rf_pulse.time;

            % Curves
            handles.axes_Mxyz = axes(handles.uipanel_Mxyz, 'OuterPosition',pos_axes_Mxyz);
            ...
            % Sphere
            handles.axes_Sphere = axes(handles.uipanel_Mxyz, 'Position',pos_axes_Sphere);
            ...
            % FFT approximation : not really related to the panel, but I need space...
            uicontrol(handles.uipanel_Mxyz,...
                'String','FFT approx Perp',...
                'Units','Normalized',...
                'Position',pos_pushbutton_fft_perp,...
                'BackgroundColor',fig_col.buttonBG,...
                'Callback', @self.callback_fft_approx_perp);
            uicontrol(handles.uipanel_Mxyz,...
                'String','FFT approx Para',...
                'Units','Normalized',...
                'Position',pos_pushbutton_fft_para,...
                'BackgroundColor',fig_col.buttonBG,...
            ...
            handles.uipanel_SliceProfile = uipanel(container,...
                'Title','SliceProfile',...
                'Units','Normalized',...
                'Position',pos_SliceProfile,...
                'BackgroundColor',fig_col.figureBG);

            pos_axes_SliceProfile = [0.00 0.10 1.05 0.90];
            handles.axes_SliceProfile = axes(handles.uipanel_SliceProfile, 'Units','normalized', 'OuterPosition',pos_axes_SliceProfile);
            p = handles.axes_SliceProfile.Position;
            pos_dZ_slider = [p(1) 0.00 p(3) 0.05];
            pos_dZ_edit   = [0.00 0.00 p(1) 0.05];
            dZ = self.app.simulation_parameters.dZ;
            dZ.add_uicontrol_select_slider(handles.uipanel_SliceProfile, pos_dZ_slider);
            dZ.add_uicontrol_select_edit  (handles.uipanel_SliceProfile, pos_dZ_edit  );
            dZ.app = self.app;
            ...
            handles.uipanel_ChemicalShift = uipanel(container,...
                'Title','ChemicalShift',...
                'Units','Normalized',...
                'Position',pos_ChemicalShift,...
                'BackgroundColor',fig_col.figureBG);
            % PPM
            handles.axes_ChemicalShiftPPM = axes(handles.uipanel_ChemicalShift, 'Units','normalized', 'OuterPosition',pos_axes_SliceProfile);
            dB0 = self.app.simulation_parameters.dB0;
            dB0.add_uicontrol_select_slider(handles.uipanel_ChemicalShift, pos_dZ_slider);
            dB0.add_uicontrol_select_edit  (handles.uipanel_ChemicalShift, pos_dZ_edit  );
            dB0.app = self.app;
